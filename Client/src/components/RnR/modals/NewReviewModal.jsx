...
export default function NewReviewModal({
        ...
        <CharContainer>
          {[1, 2, 3, 4, 5].map((num, index) => {
            return (
              ...
              >
                <input type="radio" name="characteristics" value={num} />
                {num}
              </CharButtons>
            );
          })}
  ...
  const postReviewHandler = (response) => {
    let newPhotos = [];
    const newUrlArr = response.forEach((object) =>
      newPhotos.push(object.Location)
    );
    axios
      .post("/reviews", {
        product_id: product,
  ...
  const photoUploadHandler = async (e) => {
    ...
    const res = await fetch("http://localhost:3000/photoUpload", {
      method: "POST",
      body: submittedForm,
    });
          ...
          <ModalForm
            ...
            onSubmit={(e) => {
              photoUploadHandler(e)
                .then((response) => postReviewHandler(response))
                .then(() => setShowAddReview(!showAddReview));
            }}
          >
            ...
            <React.Fragment>
              {Object.keys(metaData.characteristics).map((characteristic) =>
                characteristicsHandler(
                  characteristic,
                  metaData.characteristics[characteristic].id
                )
              )}
            </React.Fragment>
            ...
            {userReview.length < 50 ? (
              <p id="review-counter" style={{ fontSize: "12px" }}>
                Minimum required characters left: {50 - userReview.length}
              </p>
            ) : (
              <p id="review-counter" style={{ fontSize: "12px" }}>
                Minimum reached
            ...
            <ImageLabel style={{ backgroundColor: "white", color: "black" }}>
            ...
            <ModalSubmitButton
              ...
              name="submit"
            />
            <ModalButton onClick={() => setShowAddReview(!showAddReview)}>
              Cancel Review
            </ModalButton>
          </ModalForm>
        </RModalContent>
